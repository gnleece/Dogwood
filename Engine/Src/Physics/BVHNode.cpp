...
namespace DgwdPhysics
{
    ...
    BVHNode<BoundingVolumeType>::BVHNode(BVHNode* parent, Collider* collider, BoundingVolumeType& volume)
    {
        m_parent = parent;
        m_collider = collider;
        m_volume = volume;
    ...
    unsigned int BVHNode<BoundingVolumeType>::GetPotentialContactsWith(PotentialContact* contacts, unsigned int limit, BVHNode<BoundingVolumeType>* other)
    {
        ...
        BVHNode recursionNode = other;
        BVHNode nonRecursionNode = this;
    ...
    void BVHNode<BoundingVolumeType>::Insert(Collider* collider, BoundingVolumeType& volume)
    {
        ...
        if (IsLeaf())
        {
            ...
            m_collider = NULL;      // We are no longer a leaf node, so clear collider
